...
vignette: >
  %\VignetteIndexEntry{Get started with Visium Brain data}
...
colnames(data.plot)[1:3] <- c("SCA clustering", "PCA clustering", "Agreement")
...
# Heatmap
ggplot(data = data.plot, aes(x=`PCA clustering`, y=`SCA clustering`, fill=Agreement)) + 
  geom_tile() + cowplot::theme_cowplot(font_size = 20) 
```
...
Here, we will go through them in detail at whether SCA made a false discovery.
```{r session}
print(sessionInfo())
```
